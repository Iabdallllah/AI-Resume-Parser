...
def process_resume(text: str, llm):
    ...
    prompt = f"""[INST] You are a highly precise HR assistant. Extract the candidate's profile from the following resume text.
    ...
    try:
        ...
        st.error("Failed to parse the extracted data into proper JSON format.")
...
def main():
    with st.sidebar:
        ...
        with st.status("Initializing AI Engine...", expanded=True) as status:
            st.write("Loading Mistral-7B (GGUF 4-bit)...")
            llm = load_ai_model()
    ...
    st.markdown("Extract structured data from candidate CVs using **Mistral-7B GGUF**.")
